...
class GeneticAlgorithm:
    def __init__(self, num_nodes, edges, pop_size=100, mutation_rate=0.1, max_gen=1000,
                 tournament_k=5, selection_type="Tournament", crossover_type="Single Point",
                 mutation_mode="Adaptive", log_fn=print):
    ...
    def initial_population(self):
        return [[random.randint(0, self.num_nodes - 1) for _ in range(self.num_nodes)]
                for _ in range(self.pop_size)]

    def fitness(self, coloring):
        return sum(1 for u, v in self.edges if coloring[u] == coloring[v])
    ...
    def mutate(self, coloring):
        for i in range(self.num_nodes):
            if random.random() < self.mutation_rate:
                coloring[i] = random.randint(0, self.num_nodes - 1)
        return coloring
    ...
    def run(self):
        ...
        for gen in range(self.max_gen):
            ...
            save_coloring_frame(self.num_nodes, self.edges, best, gen+1, folder="frames")
            self.log(f"Gen {gen+1}: Best fitness = {best_fit}, used colors = {used_colors}")
            ...
            diverse = random.sample(population[elite_count:], min(diverse_count, len(population) - elite_count))
    ...
    def color_aware_crossover(self, p1, p2):
        ...
        for i in range(self.num_nodes):
            ...
            conflicts_c1 = sum(1 for u, v in self.edges if (u == i or v == i) and (p1[u] == p1[v]))
            conflicts_c2 = sum(1 for u, v in self.edges if (u == i or v == i) and (p2[u] == p2[v]))
    ...
    def simulated_annealing(self, coloring, initial_temp=100.0, cooling_rate=0.95, max_iter=1000):
        def cost(c):
